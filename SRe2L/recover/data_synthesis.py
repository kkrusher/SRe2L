...
device = torch.device("cuda:1")  # 指定使用GPU 1
...
def get_images(args, model_teacher, hook_for_display, ipc_id):
    ...
    for kk in range(0, 1000, batch_size):
        targets = targets_all[kk : min(kk + batch_size, 1000)].to(device)
...
def main_syn(ipc_id):
    ...
    parser.add_argument(
        "--jitter", default=32, type=int, help="random shift on the synthetic data"
    )
    parser.add_argument(
        "--r-bn",
        type=float,
    ...
    parser.add_argument(
        "--l2-scale", type=float, default=0.00001, help="l2 loss on the image"
    )
    """Model related flags"""
    parser.add_argument(
        "--arch-name",
